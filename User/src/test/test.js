...
//Get use
  ...
  //Error Signup Page
    ...
    describe('/Get ticket of user', () => {
        it('Gives the ticket info', (done) => {
        chai.request(server)
            .get('/users/ticket/eJzTd9cPCYpwCvICAAvXAoI%3D')
            .set({"Authorization": `Bearer ${userOne.tokens[0].token}`})
            // .send(user)
            .end((err, res) => {
                    res.should.have.status(200);
    ...
    describe('/Get all ticket of user', () => {
        it('Gives the all ticket of user', (done) => {
        chai.request(server)
            .get('/users/allticket')
            .set({"Authorization": `Bearer ${userOne.tokens[0].token}`})
            // .send(user)
    ...
    describe('/ delete User TICKET data', () => {
        it('it should Delete the user ticket', (done) => {
        chai.request(server)
            .delete('/users/deleteticket')
            // .set({ "Authorization": `Bearer ${token}` })
            .set({"Authorization": `Bearer ${userOne.tokens[0].token}`})
            .send({ticketId:"eJzTd9cPibQwDvEGAAtSAl0%3D"})
            .end((err, res) => {
                    res.should.have.status(200);
                done();
            });
